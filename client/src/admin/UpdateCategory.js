...
import { updateCategories, getCategory } from "./helper/adminapicall";
...
const UpdateCategory = ({ match }) => {
  ...
  const onUpdate = (event) => {
    ...
    updateCategories(match.params.categoryId, user._id, token, { name })
      .then((data) => {
        console.log(data);
        if (data.error) {
          setError(data.error);
  ...
  const warningMessage = () => {
    if (error) {
      return (
        <h4 className="text-danger">Failed to Update Category! {error}</h4>
      );
    }
  };
    ...
    return (
      <form>
        <div className="form-group">
          ...
          />
          <button onClick={onUpdate} className="btn btn-outline-primary btn-lg">
            Update Category
          </button>
        </div>
      </form>
